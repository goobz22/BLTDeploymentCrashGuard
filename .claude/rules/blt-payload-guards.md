...
`Payload/MovementOrderTypeInitGuard.cs:11-44`). The payload hot-reloads as a whole assembly, so
...
1. **Latch.** `if (_applied) return;` at the top; set `_applied = true` only after the patches
   ...
   (`Payload/PayloadEntry.cs:115-124`; `SiegeCommandGuard.RetryBt`, `SiegeCommandGuard.cs:142-155`).
...
- **One tag per guard**, a `private const string Tag` used on every line — `[SIEGE-CMD]`,
  `[COOP-CMD]`, `[MO-INIT]`, `[TIME-GUARD]`, `[GATE]`, `[IDENTITY]`… (`SiegeCommandGuard.cs:57`,
  `CoopCommandSplit.cs:46`). The tag is the grep handle; register it in the README tag legend and
  in `docs/FIX-REFERENCE.md`'s log-tag index.
...
on a hot-reload; they need a fresh launch.

## Co-op scoping

Scope every behaviour change by role through `PeerDetection` — `IsClient()`,
`AnyRemotePeerConnected()`, `ReadCoopStaticString(...)` (`Payload/BattleMode.cs:385-390`). Both
...
3. `README.md` — a numbered item under the right section, plus the tag in the legend.
4. `docs/FIX-REFERENCE.md` — a full entry (README item · Source · Class · Tag · Config · Scope, then
   Mechanism / Patched members / Limitations / Self-test) and the index rows.
