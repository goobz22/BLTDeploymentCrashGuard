...
| `[IDENTITY] SWAP AT SOURCE: Player control assigned to another hero's agent … the caller is the bug:` | The co-op identity swap caught in the act, always-on (`Payload/PlayerIdentityGuard.cs` § `ControllerPrefix`). Vanilla grants player control only to `Game.PlayerTroop` (`docs/ENGINE-NOTES.md` § *Agent identity and control*), so this fires only when something else makes another hero the player; the **live** stack under it names that caller. Once per agent; fire id `player-identity-swap-source`. The once-a-second corrector's own `[IDENTITY] player control is on the wrong agent …` line follows if it had to act. |
| `[TICK-GUARD] … took Nms (frame dt X.XXXs; budget …)` | The throttle line now carries the frame delta BannerlordTogether fed that tick; a large `dt` beside a heavy tick is the feedback loop described in `docs/ENGINE-NOTES.md` § *The campaign-tick hot stack*. |
