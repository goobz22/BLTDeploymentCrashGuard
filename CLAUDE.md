...
- **Harness** (`Harness/` → `BLTDeploymentCrashGuard.dll`) — the stable module Bannerlord loads:
  lifecycle, `Log`, health/self-test, `GuardConfig`, hot-reload engine. Its DLL is **locked while
  the game runs**; changing it needs a restart.
...
manual and the easiest of the three artifacts to forget. Never version anything else.

**Pushing == releasing.** `install.cmd` pulls `dist/`, and the README one-liners curl `install.cmd`,
`share-log.cmd` and `collect-diagnostics.cmd` from the **repo root of `main`** — those three are
release artifacts, not tooling. Their Steam auto-detect path list is copy-pasted into all three (edit
all three; the collector's copy has already drifted).
...
module root) **and** to `dist/`, then `md5sum` all three across build output, game module and
`dist/` in one pass. Mandatory because **nothing cross-checks the harness/payload pair** — not the
installer (three independent curls), not load time (the payload's `AssemblyVersion` is a per-build
wildcard); a half-updated `dist/` ships a mismatch silently. Game bin:
...
**must** be listed, it is the only thing telling players to update; a README config-table row for a
new key and tag row for a new log tag; `docs/ENGINE-NOTES.md` for a newly IL-proven fact; a
...
**While the game runs**: deploy the payload only (hot-reloads via shadow copy, `[HOTRELOAD] gen2` in
the log). Harness changes and load-time fixes need a **fresh launch**.
...
- Runtime: `"tracing": true` in `guardconfig.json`, reproduce, read `CrashGuard.log`. A session-wide
  ...
  memory and the **live** stack. A failed type initializer is **cached** — a logged throw can be a
  re-throw whose context differs from the origin.
...
- Static class, `Apply(Harmony)`, **idempotent** (`if (_applied) return;` — hot-reload re-applies).
  ...
  load-bearing fix, since it also warns on the player's screen. Registers a self-test
  (`SelfHealing.RegisterTest`), owns a log **tag** (`[SIEGE-CMD]`); per-mission state resets in
  `OnMissionInit`.
- **Every `Apply` exit path reports**, "target type not found" included — a silent return is absent
  from `MOD HEALTH:`, reading as *not built* rather than *not applicable*. A config-disabled guard
  logs the vanilla consequence and reports `(component, true, "disabled by config")`: off on purpose
  is healthy, not red. **Tracers are the deliberate exception** — a tracer's health report is its
  load line (`tracer active on N method(s)`, `type not found: X`), so print the resolved count: with
  by-name reflection a silent miss is indistinguishable from "the bug did not happen".
...
- **Session state comes from `PeerDetection`** (`Payload/BattleMode.cs:390`), never hand-rolled
  ...
  `AnyRemotePeerConnected() != false`, because a wrong "alone" sabotages a live session. A new battle
...
- A **wire/sync** feature also: keeps model + framing in engine-free files linked into
  ...
  shared `0x00` marker, proving cross-feature discrimination headlessly *and* in an in-game loopback;
  registers that self-test **before** the enabled check; resolves BT types via the candidate-name
  list, reporting DEGRADED not throwing; parses bytes only in the receive prefix, queueing work for
  `Tick`; subscribes `CampaignEvents` in `OnGameStart` keyed on `Campaign.Current`, never in `Apply`.
- A load-time fix goes **first** in `PayloadEntry.Apply`, before `PatchAll`.
- Several shipped components predate these rules and report no health at all (attribute-based
  deployment guards, `BattleMode`/`PeerDetection`, `PayloadEntry`, `PlayerIdentityGuard`,
  `BootstrapWatch`…). The list and what each one hides is in `docs/DIAGNOSTICS.md` § *What
  `MOD HEALTH:` does not cover* — read it before trusting the summary; wire new code up rather than
  widen the exception.
...
- **Do not push mid-investigation** — a push releases to players. Deploy locally, iterate, push once
  the fix is proven; the owner says when to ship.
...
- `docs/FIX-REFERENCE.md` — per-fix developer table (file/class/tag/config/scope/patched
  members/limitations/self-test) + indexes. `docs/MODDING-GUIDE.md` — public Bannerlord/BT
